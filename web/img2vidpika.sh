...
idvid=$(curl -H "Authorization: Bearer $tok" -F promptText="$2" -F image=@$1 -F options='{"frameRate":24,"camera":{},"parameters":{"guidanceScale":5,"motion":1,"negativePrompt":"takling, morphing, poor quality, motion blur, distorted"},"extend":false}' -F userId=5f7b4b85-2a8f-4eb4-9953-4ac1978a3863 https://api.pika.art/generate | jq .data.id | tr -d '"' | tr -d '\n'  )
